...
function nextRight(w: Work): Work {
    return w.children.length > 0 ? w.children[w.children.length - 1] : (w.thread as Work);
}
...
function apportion(w: Work, defaultAncestor: Work): Work {
    ...
    while (nextRight(vInnerLeft) && nextLeft(vInnerRight)) {
        ...
        const shift = vInnerLeft.prelim + sInnerLeft - (vInnerRight.prelim + sInnerRight) + DISTANCE;
        if (shift > 0) {
