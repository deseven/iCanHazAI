...
function ChatApp() {
        ...
        setHostSubscriber((msg: HostMessage) => {
            switch (msg.type) {
                ...
                case "streaming":
                    ...
                    setSnapshot((s) => (s && s.chatId === msg.chatId ? { ...s, isStreaming: msg.isStreaming } : s));
                    setTick((t) => t + 1);
                    break;
                case "theme":
                ...
                case "updateMessage":
                    debugLog(
                        ...
                        `updateMessage id=${msg.message.id} role=${msg.message.role} len=${msg.message.content.length}` +
                            (msg.message.toolCalls?.length ? ` toolCalls=${msg.message.toolCalls.length}` : "") +
                            (msg.message.toolResults?.length ? ` toolResults=${msg.message.toolResults.length}` : ""),
                    );
                    setSnapshot((s) => {
                        if (!s || s.chatId !== msg.chatId) return s;
                        const messages = s.messages.map((m) => (m.id === msg.message.id ? msg.message : m));
                        return { ...s, messages };
                    });
                    setTick((t) => t + 1);
                    setSearchTick((t) => t + 1);
    ...
    useEffect(() => {
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                const el = messageListRef.current?.querySelector(`[data-message-id="${id}"]`);
                if (el) {
    ...
    const streamingId = isStreaming && messages.length > 0 ? messages[messages.length - 1].id : null;
            ...
            {searchOpen && (
                <div class="search-bar">
                    ...
                    <span class="search-count">
                        {searchTotal > 0 ? `${searchActive} / ${searchTotal}` : searchQuery.trim() ? "0 / 0" : ""}
                    </span>
                    <button
                        type="button"
                    ...
                    <button type="button" class="search-close-btn" title="Close" onClick={() => setSearchOpen(false)}>
                        <X size={16} />
                    </button>
                </div>
            )}
