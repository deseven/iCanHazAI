...
///
/// All counts are cumulative for a single assistant response (one LLM call).
/// The totals match what the provider's dashboard reports.
struct TokenUsage: Sendable, Equatable, Codable {
    /// Total tokens consumed by the model: input + cached input + output.
    let tokensUsed: Int
    /// Input tokens that were not read from cache (new / non-cached).
    let inputTokens: Int
    /// Output tokens generated by the model.
    let outputTokens: Int
    /// Input tokens read from the provider's cache. These are processed by
    /// the model just like regular input tokens.
    let cachedInputTokens: Int
    /// Input tokens written to the provider's cache (Anthropic only).
    /// This is a billing metric, not tokens processed by the model.
    ...
    /// A human-readable summary for the chat info panel.
