...
extension AllAppTests {
    ...
    struct AttachmentTests {
        ...
        func intakeClassification() throws {
            ...
            let textAtt = AttachmentManager.intake(
                data: text, originalName: "note.txt", hint: .init(filename: "note.txt"))
            ...
            let rtfAtt = AttachmentManager.intake(
                data: rtfData, originalName: "doc.rtf", hint: .init(filename: "doc.rtf"))
            #expect(rtfAtt?.kind == .document(.rtf))

            // PNG → .image
            let png = makePNG(text: "IMG")
            let imgAtt = AttachmentManager.intake(
                data: png, originalName: "shot.png", hint: .init(filename: "shot.png"))
        ...
        func requestBuilderNoText() {
            ...
            let failed = AppAttachment(
                kind: .document, ext: "docx", originalName: "bad.docx", text: nil, status: .failed,
        ...
        func dedupNoCollision() {
            let result = AttachmentManager.deduplicatedFilename(
                proposed: "report.docx", chatFilename: dedupChatFilename)
