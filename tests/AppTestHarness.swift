...
enum Fixtures {
    ...
    /// must end up as the last message of its branch array (tree invariant).
    static func forking(_ msg: ChatMessage, branches: [[ChatMessage]], active: Int? = nil) -> ChatMessage {
        var msg = msg
        msg.branches = branches
        msg.activeBranch = active
        return msg
    }
