...
extension AllAppTests {
    ...
    struct ChatRenderQueueTests {
        ...
        private func job(
            _ chatId: String, _ messages: [ChatMessage], streaming: Bool = false,
            features: ChatSnapshotFeaturesData = ChatSnapshotFeaturesData()
        ) -> RenderJob {
            let chat = Chat(messages: messages)
            return .snapshot(
                chatId: chatId, chat: chat, isStreaming: streaming, roleName: nil, roleAccent: nil, features: features)
        }
