...
extension AllAppTests {
    ...
    struct WorkdirItemsBuilderTests {
        ...
        func filteredRecents() {
            ...
            #expect(
                WorkdirItemsBuilder.build(query: "website", recents: recents, typedDir: nil, children: [])
                    == [.recent("test:/var/www/website")])
            // "~" matches the local recent via its tilde-abbreviated display form.
            #expect(
                WorkdirItemsBuilder.build(query: "~", recents: recents, typedDir: nil, children: [])
        ...
        func ordering() {
            ...
            let items = WorkdirItemsBuilder.build(
                query: "~/w", recents: recents, typedDir: home + "/weird", children: children)
            #expect(
                items == [.directory(home + "/weird"), .directory(home + "/whatever"), .directory(home + "/wombat")])
        }
        ...
        func perfectMatchSuppressesTyped() {
            let home = NSHomeDirectory()
            let items = WorkdirItemsBuilder.build(
                query: "~/my-project", recents: recents, typedDir: home + "/my-project", children: [])
    ...
    struct WorkdirLocalListingTests {
        ...
        private func makeTree() throws -> URL {
            let root = URL(
                fileURLWithPath: (FileManager.default.temporaryDirectory
                    .appendingPathComponent("ichai-picker-test-" + UUID().uuidString, isDirectory: true).path
        ...
        func prefixFiltering() throws {
            ...
            defer { try? FileManager.default.removeItem(at: root) }
            #expect(
                WorkdirQuery.localChildren(dir: root.path, prefix: "A") == [root.appendingPathComponent("alpha").path])
            #expect(
                WorkdirQuery.localChildren(dir: root.path, prefix: "b") == [root.appendingPathComponent("Beta").path])
        ...
        func hiddenEntries() throws {
            ...
            defer { try? FileManager.default.removeItem(at: root) }
            #expect(
                WorkdirQuery.localChildren(dir: root.path, prefix: ".") == [root.appendingPathComponent(".hidden").path]
            )
        }
