    ...
    struct ChatModelTests {
        ...
        func displayTitleLoadedFirstUser() {
            let rec = ChatRecord(
                ...
                chat: Fixtures.chat(
                    messages: [
                        Fixtures.message(role: .assistant, content: "hi"),
        ...
        func tokenCountSumsAll() {
            let chat = Fixtures.chat(messages: [
                Fixtures.message(
                    role: .assistant, content: "old", timestamp: Date(timeIntervalSince1970: 1),
                    tokenUsage: TokenUsage(tokensUsed: 10)),
                Fixtures.message(role: .user, content: "again", timestamp: Date(timeIntervalSince1970: 2)),
                Fixtures.message(
                    role: .assistant, content: "new", timestamp: Date(timeIntervalSince1970: 3),
